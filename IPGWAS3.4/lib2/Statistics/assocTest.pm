...
sub assocTests
{
my($output_text, $mw, $gp_win, $Assoc)=@_;
...
#option frame
	my $opt_frame=$gp_win->LabFrame(-label=>"Model", -labelside=>'acrosstop')->pack(-side=>"top", -anchor=>'w'); #option frame
		my $opt1_frame=$opt_frame->Frame(-borderwidth=>1)->pack(-side => "top");
			my $radio1=$opt1_frame->Radiobutton(-text=>"Best          ", -value=>"best", -variable=>\$model)->pack(-side => "left", -anchor=>'w');	
			...
			my $radio4=$opt1_frame->Radiobutton(-text=>"Allelic     ", -value=>"allelic", -variable=>\$model)->pack(-side => "left", -anchor=>'w');
			my $radio5=$opt1_frame->Radiobutton(-text=>"Genotypic   ", -value=>"geno", -variable=>\$model)->pack(-side => "left", -anchor=>'w');
#output name frame
...
$ok_frame->Button(-text => "OK",-command =>sub{
												...
												my $log9="$outname.log";
												#----------------------------------------------------------------------------------
												system("$Assoc --ped $ped --map $map --model $model --out $outname --log $log9");
